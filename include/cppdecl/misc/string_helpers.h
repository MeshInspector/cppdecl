...
namespace cppdecl
{
    ...
    // Splits `input` by string and character literals, and calls `func` on the remaining substrings.
    // Additionally, when seeing a suspiciously long character literal (more than one character and not an escape sequence), the entire literal with single quotes
    //   is also passed to a separate call to `func`.
    // `func` is `(std::string_view part) -> bool`. If it returns true, the whole function stops and also returns true. Or if this never happened, returns false.
    // Always calls `func` at least once. Can call it on empty strings.
    // On an unterminated literal, immediately returns false and writes a non-null error message to `*error`. Otherwise writes null to `*error`.
    constexpr int VisitEverythingButStringAndCharLiterals(std::string_view input, auto &&func, const char **error = nullptr)
    {
        if (error)
            *error = nullptr;

        std::string_view begin = input;

        while (true)
        {
            // Remove characters until a literal or until the end of input.
            while (!input.empty() && input.front() != '"' && input.front() != '\'')
                input.remove_prefix(1);

            // Call `func()` on the consumed characters.
            if (func(std::string_view(begin.data(), std::size_t(input.data() - begin.data()))))
                return true;

            // Stop if nothing else to consume.
            if (input.empty())
                return false;

            // Start consuming the literal.
            char quote = input.front();
            std::string_view input_at_literal = input;
            input.remove_prefix(1);
            ...
            while (true)
            {
                if (input.starts_with('\\'))
                {
                    ...
                    // Check for incomplete escape sequence at the end of input.
                    if (input.empty())
                    {
                        if (error)
                            *error = "Incomplete escape sequence at the end of input.";
                        return false;
                    }
                ...
                else
                {
                    ...
                    if (input.starts_with(quote))
                    {
                        ...
                        if (quote == '\'')
                        {
                            std::string_view contents(input_at_literal.data(), std::size_t(input.data() - input_at_literal.data()));
                            if (contents.size() > 3 && contents[1] != '\\')
                            {
                                if (func(contents))
                                    return true;
                            }
                    ...
                    {
                        if (error)
                            *error = quote == '\'' ? "Unterminated character literal." : "Unterminated string literal.";
                        return false;
                    }

                    // Just some unknown character.
                    input.remove_prefix(1);
                }
            }
        }
